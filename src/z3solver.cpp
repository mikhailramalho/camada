...
Z3Solver::Z3Solver()
    : SMTSolverImpl(), Context(new z3::context()), Solver(*Context) {
  Context->set_enable_exceptions(false);
...
SMTSortRef Z3Solver::newSortRefImpl(const SMTSortRef &Sort) const {
  return Sort;
}

SMTExprRef Z3Solver::newExprRefImpl(const SMTExprRef &Exp) const { return Exp; }

SMTSortRef Z3Solver::mkBoolSortImpl() {
  return newSortRef(
      new SolverBoolSort<Z3Sort>({Context, Context->bool_sort()}));
}

SMTSortRef Z3Solver::mkBVSortImpl(unsigned BitWidth) {
  return newSortRef(new SolverBVSort<Z3Sort>(
      {BitWidth, Context, Context->bv_sort(BitWidth)}));
}

SMTSortRef Z3Solver::mkRMSortImpl() {
  return newSortRef(new SolverRMSort<Z3Sort>(
      {Context,
       z3::to_sort(*Context, Z3_mk_fpa_rounding_mode_sort(*Context))}));
}

SMTSortRef Z3Solver::mkFPSortImpl(const unsigned ExpWidth,
                                  const unsigned SigWidth) {
  return newSortRef(
      new SolverFPSort<Z3Sort>({ExpWidth, SigWidth, Context,
...
SMTSortRef Z3Solver::mkBVFPSortImpl(const unsigned ExpWidth,
                                    const unsigned SigWidth) {
  return newSortRef(
      new SolverBVFPSort<Z3Sort>({ExpWidth, SigWidth + 1, Context,
                                  Context->bv_sort(ExpWidth + SigWidth + 1)}));
}

SMTSortRef Z3Solver::mkBVRMSortImpl() {
  return newSortRef(new SolverBVRMSort<Z3Sort>({Context, Context->bv_sort(3)}));
}

SMTSortRef Z3Solver::mkArraySortImpl(const SMTSortRef &IndexSort,
                                     const SMTSortRef &ElemSort) {
  return newSortRef(new SolverArraySort<Z3Sort>(
...
SMTExprRef Z3Solver::mkBVNegImpl(const SMTExprRef &Exp) {
  return newExprRef(
      new Z3Expr(Context, Exp->Sort, -toSolverExpr<Z3Expr>(*Exp).Expr));
}
...
SMTExprRef Z3Solver::mkBVAddImpl(const SMTExprRef &LHS, const SMTExprRef &RHS) {
  return newExprRef(new Z3Expr(Context, LHS->Sort,
...
SMTExprRef Z3Solver::mkIteImpl(const SMTExprRef &Cond, const SMTExprRef &T,
                               const SMTExprRef &F) {
  return newExprRef(new Z3Expr(Context, T->Sort,
...
SMTExprRef Z3Solver::mkBVSignExtImpl(unsigned i, const SMTExprRef &Exp) {
  return newExprRef(new Z3Expr(Context, mkBVSort(i + Exp->getWidth()),
...
SMTExprRef Z3Solver::mkArraySelectImpl(const SMTExprRef &Array,
  ...
  return newExprRef(new Z3Expr(Context, Array->Sort->getElementSort(),
...
SMTExprRef Z3Solver::mkArrayStoreImpl(const SMTExprRef &Array,
  ...
  return newExprRef(new Z3Expr(Context, Array->Sort,
...
SMTExprRef Z3Solver::mkFPtoSBVImpl(const SMTExprRef &From, unsigned ToWidth) {
  ...
  return newExprRef(new Z3Expr(
      ...
      z3::to_expr(
          *Context,
          Z3_mk_fpa_to_sbv(*Context, toSolverExpr<Z3Expr>(*roundingMode).Expr,
...
SMTExprRef Z3Solver::mkFPtoUBVImpl(const SMTExprRef &From, unsigned ToWidth) {
  ...
  return newExprRef(new Z3Expr(
      ...
      z3::to_expr(
          *Context,
          Z3_mk_fpa_to_ubv(*Context, toSolverExpr<Z3Expr>(*roundingMode).Expr,
                           toSolverExpr<Z3Expr>(*From).Expr, ToWidth))));
}
...
static inline SMTExprRef getZ3Interp(const Z3Solver &S, const SMTExprRef &Exp) {
  return S.newExprRef(new Z3Expr(S.Context, Exp->Sort,
...
SMTExprRef Z3Solver::mkBVFromBinImpl(const std::string &Int,
  ...
  SMTExprRef bv =
      newExprRef(new Z3Expr(Context, Sort, Context->bv_val(s, newInt)));
  delete[] newInt;
  return bv;
}
...
SMTExprRef Z3Solver::mkFPFromBinImpl(const std::string &FP, unsigned EWidth) {
  ...
  return newExprRef(new Z3Expr(
      Context, mkFPSort(EWidth, FP.length() - EWidth - 1),
      z3::to_expr(*Context,
