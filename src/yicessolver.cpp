...
YicesSort::~YicesSort() { yices_decref_type(Sort); }
...
YicesExpr::~YicesExpr() { yices_decref_term(Expr); }
...
SMTSortRef YicesSolver::newSortRefImpl(const SMTSortRef &Sort) const {
  assert(toSolverSort<YicesSort>(*Sort).Sort != NULL_TYPE &&
         "Error when creating Yices sort.");
  return Sort;
}

SMTExprRef YicesSolver::newExprRefImpl(const SMTExprRef &Exp) const {
...
SMTSortRef YicesSolver::mkBoolSortImpl() {
  return newSortRef(
      new SolverBoolSort<YicesSort>({Context, yices_bool_type()}));
}

SMTSortRef YicesSolver::mkBVSortImpl(unsigned BitWidth) {
  return newSortRef(new SolverBVSort<YicesSort>(
...
SMTSortRef YicesSolver::mkBVFPSortImpl(const unsigned ExpWidth,
  ...
  return newSortRef(
      new SolverBVFPSort<YicesSort>({ExpWidth, SigWidth + 1, Context,
                                     yices_bv_type(ExpWidth + SigWidth + 1)}));
}

SMTSortRef YicesSolver::mkBVRMSortImpl() {
  return newSortRef(new SolverBVRMSort<YicesSort>({Context, yices_bv_type(3)}));
}

SMTSortRef YicesSolver::mkArraySortImpl(const SMTSortRef &IndexSort,
                                        const SMTSortRef &ElemSort) {
  return newSortRef(new SolverArraySort<YicesSort>(
...
SMTExprRef YicesSolver::mkBVNegImpl(const SMTExprRef &Exp) {
  return newExprRef(new YicesExpr(
      Context, Exp->Sort, yices_bvneg(toSolverExpr<YicesExpr>(*Exp).Expr)));
}

SMTExprRef YicesSolver::mkBVNotImpl(const SMTExprRef &Exp) {
  return newExprRef(new YicesExpr(
      Context, Exp->Sort, yices_bvnot(toSolverExpr<YicesExpr>(*Exp).Expr)));
}

SMTExprRef YicesSolver::mkNotImpl(const SMTExprRef &Exp) {
  return newExprRef(new YicesExpr(
      Context, Exp->Sort, yices_not(toSolverExpr<YicesExpr>(*Exp).Expr)));
}

SMTExprRef YicesSolver::mkBVAddImpl(const SMTExprRef &LHS,
                                    const SMTExprRef &RHS) {
  return newExprRef(
      new YicesExpr(Context, LHS->Sort,
...
SMTExprRef YicesSolver::mkIteImpl(const SMTExprRef &Cond, const SMTExprRef &T,
                                  const SMTExprRef &F) {
  return newExprRef(new YicesExpr(Context, T->Sort,
...
SMTExprRef YicesSolver::mkBVSignExtImpl(unsigned i, const SMTExprRef &Exp) {
  return newExprRef(
      new YicesExpr(Context, mkBVSort(i + Exp->getWidth()),
...
SMTExprRef YicesSolver::mkBVConcatImpl(const SMTExprRef &LHS,
                                       const SMTExprRef &RHS) {
  return newExprRef(
      new YicesExpr(Context, mkBVSort(LHS->getWidth() + RHS->getWidth()),
...
SMTExprRef YicesSolver::mkBVRedOrImpl(const SMTExprRef &Exp) {
  return newExprRef(new YicesExpr(
      Context, mkBVSort(1), yices_redor(toSolverExpr<YicesExpr>(*Exp).Expr)));
}
...
SMTExprRef YicesSolver::mkArraySelectImpl(const SMTExprRef &Array,
  ...
  return newExprRef(
      new YicesExpr(Context, Array->Sort->getElementSort(),
...
SMTExprRef YicesSolver::mkArrayStoreImpl(const SMTExprRef &Array,
  ...
  return newExprRef(
      new YicesExpr(Context, Array->Sort,
...
SMTExprRef YicesSolver::mkBoolImpl(const bool b) {
  return newExprRef(
      new YicesExpr(Context, mkBoolSort(), b ? yices_true() : yices_false()));
}

SMTExprRef YicesSolver::mkBVFromDecImpl(const int64_t Int,
                                        const SMTSortRef &Sort) {
  return newExprRef(
      new YicesExpr(Context, Sort, yices_bvconst_int64(Sort->getWidth(), Int)));
}

SMTExprRef YicesSolver::mkBVFromBinImpl(const std::string &Int,
                                        const SMTSortRef &Sort) {
  return newExprRef(
      new YicesExpr(Context, Sort, yices_parse_bvbin(Int.c_str())));
}
...
SMTExprRef YicesSolver::mkSymbolImpl(const std::string &Name,
  ...
  auto inserted = SymbolTable.insert(SymbolTablet::value_type(
      Name, newExprRef(new YicesExpr(Context, Sort, t))));
