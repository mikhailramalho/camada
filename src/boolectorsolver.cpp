...
BtorSort::~BtorSort() { boolector_release_sort(Context, Sort); }

unsigned BtorSort::getWidthFromSolver() const {
  return boolector_bitvec_sort_get_width(Context, Sort);
}

BtorExpr::~BtorExpr() { boolector_release(Context, Expr); }
...
void BtorExpr::dump() const { boolector_dump_smt2_node(Context, stderr, Expr); }
...
BtorSolver::BtorSolver() : SMTSolverImpl(), Context(boolector_new()) {
...
SMTExprRef BtorSolver::mkBVUltImpl(const SMTExprRef &LHS,
                                   const SMTExprRef &RHS) {
  return newExprRef(
      new BtorExpr(Context, mkBoolSort(),
...
SMTExprRef BtorSolver::mkBVSignExtImpl(unsigned i, const SMTExprRef &Exp) {
  return newExprRef(new BtorExpr(
...
SMTExprRef BtorSolver::mkBVConcatImpl(const SMTExprRef &LHS,
                                      const SMTExprRef &RHS) {
  return newExprRef(
      new BtorExpr(Context, mkBVSort(LHS->getWidth() + RHS->getWidth()),
...
SMTExprRef BtorSolver::mkArraySelectImpl(const SMTExprRef &Array,
  ...
  return newExprRef(
      new BtorExpr(Context, Array->Sort->getElementSort(),
...
SMTExprRef BtorSolver::mkArrayStoreImpl(const SMTExprRef &Array,
  ...
  return newExprRef(
      new BtorExpr(Context, Array->Sort,
...
SMTExprRef BtorSolver::mkBVFromBinImpl(const std::string &Int,
                                       const SMTSortRef &Sort) {
  return newExprRef(
      new BtorExpr(Context, Sort, boolector_const(Context, Int.c_str())));
}
