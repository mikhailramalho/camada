...
STPSolver::STPSolver(STPContext C) : SMTSolverImpl(), Context(std::move(C)) {}
...
SMTSortRef STPSolver::newSortRefImpl(const SMTSortRef &Sort) const {
  return Sort;
}

SMTExprRef STPSolver::newExprRefImpl(const SMTExprRef &Exp) const {
  return Exp;
}

SMTSortRef STPSolver::mkBoolSortImpl() {
  return newSortRef(
      new SolverBoolSort<STPSort>({Context, STP::vc_boolType(Context)}));
}

SMTSortRef STPSolver::mkBVSortImpl(unsigned BitWidth) {
  return newSortRef(new SolverBVSort<STPSort>(
      {BitWidth, Context, STP::vc_bvType(Context, BitWidth)}));
}

SMTSortRef STPSolver::mkBVFPSortImpl(const unsigned ExpWidth,
                                     const unsigned SigWidth) {
  return newSortRef(new SolverBVFPSort<STPSort>(
...
SMTSortRef STPSolver::mkBVRMSortImpl() {
  return newSortRef(
      new SolverBVRMSort<STPSort>({Context, STP::vc_bvType(Context, 3)}));
}

SMTSortRef STPSolver::mkArraySortImpl(const SMTSortRef &IndexSort,
                                      const SMTSortRef &ElemSort) {
  return newSortRef(new SolverArraySort<STPSort>(
...
SMTExprRef STPSolver::mkArraySelectImpl(const SMTExprRef &Array,
  ...
  return newExprRef(
      new STPExpr(Context, Array->Sort->getElementSort(),
...
SMTExprRef STPSolver::mkArrayStoreImpl(const SMTExprRef &Array,
  ...
  return newExprRef(
      new STPExpr(Context, Array->Sort,
...
SMTExprRef STPSolver::mkBVFromBinImpl(const std::string &Int,
                                      const SMTSortRef &Sort) {
  return newExprRef(new STPExpr(
      Context, Sort, STP::vc_bvConstExprFromStr(Context, Int.c_str())));
}
